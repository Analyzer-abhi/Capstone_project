...
export async function searchJobsAgent({ skills, experience }) {
  ...
  // If no search results, return empty array - NO PROTOTYPE DATA
  if (allJobListings.length === 0) {
    console.warn('   ⚠️  No job listings found from internet search');
    console.log('========================================\n');
    return [];
  }

  const searchContext = allJobListings.join('\n\n').slice(0, 12000); // Increased limit for more data
  console.log(`   ✅ Successfully compiled ${searchContext.length} characters of search data`);

  console.log(`\n🤖 Step 4: Analyzing data with AI (Gemini 2.5 Flash)...`);
  console.log(`   Extracting job listings from search results...`);

  const prompt = `You are a job data extractor. Extract ONLY real, active job listings from the search results below. DO NOT generate, create, or add any fake/prototype/example jobs.

User Skills: ${skills.join(', ')}
User Experience: ${experience}

REAL JOB LISTINGS FROM INTERNET (from multiple sources):
${searchContext}

CRITICAL INSTRUCTIONS - READ CAREFULLY:
1. Extract ONLY jobs that actually appear in the search results above
2. DO NOT create, generate, invent, or add any job listings that aren't in the search results
3. Extract EXACT company names, job titles, and URLs from the results
4. Only include jobs that are currently active and accepting applications
5. If you cannot find real jobs in the results, return empty array [] - DO NOT make up data
6. Extract real URLs from ANY job board or company career page that appears in results
7. Accept URLs from: LinkedIn, Indeed, Glassdoor, company career pages, startup sites, remote job boards, etc.
8. Match scores should be based on actual job requirements mentioned in the listing vs user skills
9. For experience level: If job mentions "entry level", "junior", "0-2 years" → it's entry level. Otherwise → experienced level
10. DO NOT add generic job titles like "Software Engineer at Tech Corp" - only real companies
11. DO NOT use placeholder data like "Tech Corp", "Startup Inc", "Company XYZ"
12. Extract jobs from ALL sources in the search results, not just major job boards
