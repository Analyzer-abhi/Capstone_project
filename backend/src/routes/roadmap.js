...
import { createRoadmap, getRoadmapById, updateProgress } from '../services/roadmapService.js';
...
router.post('/analyze', upload.single('resume'), async (req, res) => {
  try {
    ...
    // Agentic workflow: 1 → 2 → 3
    const resumeSkills = await extractResumeSkills(resumeText);
    const jobRequirements = await fetchJobRequirements(targetJobTitle);
...
router.get('/:id', async (req, res) => {
  try {
    const doc = await getRoadmapById(req.params.id);
...
router.patch('/:id/progress', async (req, res) => {
  try {
    ...
    const updated = await updateProgress(req.params.id, unique);
    res.json({
      id: updated.id,
      progress: updated.progress,
      allPhasesCompleted: updated.allPhasesCompleted,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
